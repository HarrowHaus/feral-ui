...
function StyleLabPage() { return <RoutePage hero={false} eyebrow="Style Lab" title="Style Lab" description="Pick a preset, tweak palette roles, preview real components, then copy CSS."><StyleLab /></RoutePage>; }
function OrnamentsPage() { return <RoutePage hero={false} eyebrow="Ornaments" title="Ornaments" description="Every exported ornament renders from one paired name/component catalog."><OrnamentsRoute /></RoutePage>; }
...
function RoutePage({ eyebrow, title, description, hero = true, children }: { eyebrow: string; title: string; description: string; hero?: boolean; children: React.ReactNode }) { return <section className="route-page">{hero ? <div className="route-hero"><Badge tone="pink">{eyebrow}</Badge><h1>{title}</h1><p>{description}</p></div> : null}{children}</section>; }
...
function Router({ route }: { route: string }) { const parts = route.split("/").filter(Boolean); if (route === "/" || route === "") return <HomePage />; if (parts[0] === "docs") return <DocsPage slug={parts[1]} />; if (parts[0] === "components") return <ComponentsPage slug={parts[1]} />; if (parts[0] === "blocks" && !parts[1]) return <BlocksPage />; if (parts[0] === "templates") return <TemplatesPage slug={parts[1]} />; if (parts[0] === "style-lab" && !parts[1]) return <StyleLabPage />; if (parts[0] === "ornaments" && !parts[1]) return <OrnamentsPage />; if (parts[0] === "roadmap" && !parts[1]) return <RoadmapPage />; return <NotFound />; }
