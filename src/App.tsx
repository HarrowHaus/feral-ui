...
function DesktopNavControls({ onSearch }: { onSearch: () => void }) {
  return (
    <span className="site-nav-right site-nav-desktop-controls">
      <button className="site-search" type="button" onClick={onSearch}><Search size={13} /> Search <kbd>⌘K</kbd></button>
    ...
    <TooltipProvider>
      ...
      <header className="site-nav" data-feral-shell="true">
        ...
        <nav className="site-nav-links" aria-label="Primary">
          {navItems.filter((item) => item.label !== "Install" && item.label !== "Roadmap").map((item) => <a key={item.href} href={item.href}>{item.label}</a>)}
        </nav>
...
function ComponentsPage({ slug }: { slug?: string }) { if (slug) return <ComponentDetail slug={slug} />; return <RoutePage eyebrow="Components" title="66 creatures. Browse the enclosures." description="Tap the glass — they like it."><div className="route-grid">{componentCatalog.map((item) => <ComponentCard key={item.slug} item={item} />)}</div></RoutePage>; }
function ComponentDetail({ slug }: { slug: string }) { const item = componentCatalog.find((entry) => entry.slug === slug); if (!item) return <NotFound />; const content = componentContent(item); const index = componentCatalog.findIndex((entry) => entry.slug === slug); const prev = componentCatalog[index - 1]; const next = componentCatalog[index + 1]; const specimen = playgroundSlugs.has(item.slug) ? <ComponentPlayground slug={item.slug} /> : <ComponentShowcase slug={item.slug} />; return <RoutePage eyebrow={item.layer} title={item.name} description={content.description}><div className={playgroundSlugs.has(item.slug) ? "site-stack" : "route-two-col"}><Card><CardHeader><Badge tone={item.status === "built" ? "acid" : "tang"}>{statusLabel(item.status)}</Badge><CardTitle>{playgroundSlugs.has(item.slug) ? "Live specimen" : "Specimen"}</CardTitle><CardDescription>{item.file}</CardDescription></CardHeader><CardContent>{specimen}</CardContent></Card><CodeTabs tabs={[{ id: "usage", label: "Usage", code: content.usage }, { id: "install", label: "Install", code: `npx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/${item.slug}.json` }, { id: "a11y", label: "Behavior", code: content.accessibility }]} /></div><div className="route-pager">{prev ? <Button tone="paper" onClick={() => navigate(`/components/${prev.slug}`)}>← {prev.name}</Button> : <span />}{next ? <Button tone="paper" onClick={() => navigate(`/components/${next.slug}`)}>{next.name} →</Button> : <span />}</div></RoutePage>; }
function DocsPage({ slug }: { slug?: string }) { if (!slug) return <RoutePage eyebrow="Docs" title="Documentation with fewer trap doors." description="Install, customize, browse components, and keep Beige outside the fence."><div className="route-grid">{[["Installation", "Release a component into your codebase.", "/docs/installation"], ["Accessibility", "Boring semantics. Radioactive sticker sheet.", "/docs/accessibility"], ["Molt Log", "Release notes. Things grew limbs.", "/docs/changelog"]].map(([title, desc, href]) => <Card key={title}><CardHeader><CardTitle>{title}</CardTitle><CardDescription>{desc}</CardDescription></CardHeader><CardFooter><Button tone="acid" onClick={() => navigate(href)}>Open</Button></CardFooter></Card>)}</div></RoutePage>; if (slug === "installation") return <InstallationPage />; if (slug === "accessibility") return <AccessibilityPage />; if (slug === "changelog") return <ChangelogPage />; return <NotFound />; }
function InstallationPage() { return <RoutePage eyebrow="Installation" title="Releasing a component into your codebase." description="One command. It installs its own source. It knows the way home — the way home is your components/ui folder now."><div className="route-two-col"><CodeTabs tabs={[{ id: "npm", label: "npm", code: "npx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/button.json" }, { id: "pnpm", label: "pnpm", code: "pnpm dlx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/button.json" }, { id: "yarn", label: "yarn", code: "yarn dlx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/button.json" }, { id: "bun", label: "bun", code: "bunx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/button.json" }, { id: "manual", label: "Catch it by hand", code: "curl https://harrowhaus.github.io/feral-ui/r/button.json" }]} /><Card tone="pink"><CardHeader><CardTitle>Side effects include:</CardTitle><CardDescription>Visible focus rings, chromatic shadows, and your design lead asking to hop on a quick call.</CardDescription></CardHeader><CardContent><CodeBlock>{`npx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/card.json
npx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/dialog.json
npx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/data-table.json`}</CodeBlock></CardContent></Card></div></RoutePage>; }
function AccessibilityPage() { return <RoutePage eyebrow="Policy" title="Boring semantics. Radioactive sticker sheet." description="The interaction layer behaves like a professional adult with a mortgage."><div className="route-grid"><Card><CardHeader><CardTitle>Focus rings</CardTitle><CardDescription>Visible, loud, and not optional.</CardDescription></CardHeader></Card><Card><CardHeader><CardTitle>Keyboard ground</CardTitle><CardDescription>Keyboards are sacred ground. The goblin removes its shoes.</CardDescription></CardHeader></Card><Card><CardHeader><CardTitle>Radix where it matters</CardTitle><CardDescription>Dialogs and menus inherit the boring adult behavior.</CardDescription></CardHeader></Card></div></RoutePage>; }
function ChangelogPage() { return <RoutePage eyebrow="Molt Log" title="Things grew limbs." description="v0.7 — the calendar grew range selection. We do not know where it learned that."><div className="site-stack">{[["v0.7", "Calendar range mode, toast provider, carousel controls, sidebar behavior."], ["v0.6", "Routed pages, Style Lab route, ornaments, template routes."], ["v0.5", "Registry basis and docs product shell."]].map(([version, body]) => <Card key={version}><CardHeader><CardTitle>{version}</CardTitle><CardDescription>{body}</CardDescription></CardHeader></Card>)}</div></RoutePage>; }
function BlocksPage() { return <RoutePage eyebrow="Blocks" title="Page sections with bite marks." description="The block library is the forcing function for deeper components."><div className="route-grid">{["Marketing", "Dashboard", "Auth", "Content", "Commerce"].map((name) => <Card key={name}><CardHeader><Badge tone="tang">IN THE ENCLOSURE</Badge><CardTitle>{name}</CardTitle><CardDescription>{name} block family gets expanded next.</CardDescription></CardHeader></Card>)}</div></RoutePage>; }
function TemplatesPage({ slug }: { slug?: string }) { const selected = slug ? templateCatalog.find((item) => slugify(item.name) === slug) : undefined; if (slug && !selected) return <NotFound />; if (selected) return <RoutePage eyebrow="Template" title={selected.name} description={selected.description}>{selected.component}<Card tone="acid" radius="none"><CardHeader><CardTitle>Take it home</CardTitle><CardDescription>Template exit ramps are next: installable template item or generated zip, then fresh-app smoke.</CardDescription></CardHeader></Card></RoutePage>; return <RoutePage eyebrow="Templates" title="Live habitats." description="Eight routes prove the kit can build whole surfaces, not just button zoos."><div className="route-grid">{templateCatalog.map((template) => <Card key={template.name}><CardHeader><Badge tone="acid">LOOSE</Badge><CardTitle>{template.name}</CardTitle><CardDescription>{template.description}</CardDescription></CardHeader><CardFooter><Button tone="pink" onClick={() => navigate(`/templates/${slugify(template.name)}`)}>Open</Button></CardFooter></Card>)}</div></RoutePage>; }
function StyleLabPage() { return <RoutePage eyebrow="Style Lab" title="Build a look without letting the goblin drive." description="Pick a preset, tweak palette roles, preview real components, then copy CSS."><StyleLab /></RoutePage>; }
function OrnamentsPage() { return <RoutePage eyebrow="Ornaments" title="The sticker sheet escaped containment." description="Every exported ornament renders from one paired name/component catalog."><OrnamentsRoute /></RoutePage>; }
function RoadmapPage() { return <RoutePage eyebrow="Roadmap" title="Build gates, not timeline theater." description="The public homepage no longer carries the project log. This route does."><div className="site-stack">{["One AppShell", "Ornament repair", "Component playgrounds", "Sidebar/Form/Toast depth", "Charts route", "Template exit ramps", "Dark mode"].map((item) => <Card key={item}><CardHeader><CardTitle>{item}</CardTitle><CardDescription>Queued by the competitive dossier.</CardDescription></CardHeader></Card>)}</div></RoutePage>; }
function RoutePage({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: React.ReactNode }) { return <section className="route-page"><div className="route-hero"><Badge tone="pink">{eyebrow}</Badge><h1>{title}</h1><p>{description}</p></div>{children}</section>; }
function NotFound() { return <RoutePage eyebrow="404" title="This route escaped into the vents." description="No page exists here. The raccoon denies involvement."><Button tone="acid" onClick={() => navigate("/")}>Go home</Button></RoutePage>; }
function Router({ route }: { route: string }) { const parts = route.split("/").filter(Boolean); if (route === "/" || route === "") return <HomePage />; if (parts[0] === "docs") return <DocsPage slug={parts[1]} />; if (parts[0] === "components") return <ComponentsPage slug={parts[1]} />; if (parts[0] === "blocks") return <BlocksPage />; if (parts[0] === "templates") return <TemplatesPage slug={parts[1]} />; if (parts[0] === "style-lab") return <StyleLabPage />; if (parts[0] === "ornaments") return <OrnamentsPage />; if (parts[0] === "roadmap") return <RoadmapPage />; return <NotFound />; }

export default function App() { const route = useHashRoute(); useDocumentTitle(route); return <AppShell><Router route={route} /></AppShell>; }
