...
function FieldReports() {
  const reports = [["The button bit me.", "The button is up to date on its shots."], ["My design system had a panic attack.", "We sent flowers. Beige ones. As a warning."], ["I pressed ⌘K and something FETCHED.", "Good palette. Best palette."]];
...
function CTA() {
  return <section className="site-section site-cta"><h2>Release one.</h2><p>Start with Button. If it behaves, try Dialog. Do not feed Card after midnight.</p><CodeBlock>{`npx shadcn@latest add https://harrowhaus.github.io/feral-ui/r/button.json`}</CodeBlock></section>;
}

function ComponentCard({ item }: { item: typeof componentCatalog[number] }) {
  const content = componentContent(item);
  return <Card><CardHeader><Badge tone={item.status === "built" ? "acid" : "tang"}>{statusLabel(item.status)}</Badge><CardTitle>{item.name}</CardTitle><CardDescription>{content.description}</CardDescription></CardHeader><CardFooter><Button tone="paper" onClick={() => navigate(`/components/${item.slug}`)}>Open</Button></CardFooter></Card>;
}

function ComponentsPage({ slug }: { slug?: string }) {
  if (slug) return <ComponentDetail slug={slug} />;
  return <RoutePage eyebrow="Components" title="66 creatures. Browse the enclosures." description="Tap the glass — they like it."><div className="route-grid">{componentCatalog.map((item) => <ComponentCard key={item.slug} item={item} />)}</div></RoutePage>;
}
...
function TemplatesPage({ slug }: { slug?: string }) {
  ...
  if (selected) return <RoutePage eyebrow="Template" title={selected.name} description={selected.description}>{selected.component}<Card tone="acid" radius="none"><CardHeader><CardTitle>Take it home</CardTitle><CardDescription>Template exit ramps are next: installable template item or generated zip, then fresh-app smoke.</CardDescription></CardHeader></Card></RoutePage>;
  return <RoutePage eyebrow="Templates" title="Live habitats." description="Eight routes prove the kit can build whole surfaces, not just button zoos."><div className="route-grid">{templateCatalog.map((template) => <Card key={template.name}><CardHeader><Badge tone="acid">LOOSE</Badge><CardTitle>{template.name}</CardTitle><CardDescription>{template.description}</CardDescription></CardHeader><CardFooter><Button tone="pink" onClick={() => navigate(`/templates/${slugify(template.name)}`)}>Open</Button></CardFooter></Card>)}</div></RoutePage>;
}

function StyleLabPage() {
  return <RoutePage eyebrow="Style Lab" title="Build a look without letting the goblin drive." description="Pick a preset, tweak palette roles, preview real components, then copy CSS."><StyleLab /></RoutePage>;
}

function OrnamentsPage() {
  return <RoutePage eyebrow="Ornaments" title="The sticker sheet escaped containment." description="Every exported ornament renders from one paired name/component catalog."><OrnamentsRoute /></RoutePage>;
}

function RoadmapPage() {
  return <RoutePage eyebrow="Roadmap" title="Build gates, not timeline theater." description="The public homepage no longer carries the project log. This route does."><div className="site-stack">{["One AppShell", "Ornament repair", "Component playgrounds", "Sidebar/Form/Toast depth", "Charts route", "Template exit ramps", "Dark mode"].map((item) => <Card key={item}><CardHeader><CardTitle>{item}</CardTitle><CardDescription>Queued by the competitive dossier.</CardDescription></CardHeader></Card>)}</div></RoutePage>;
}
...
function RoutePage({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: React.ReactNode }) {
  return <section className="route-page"><div className="route-hero"><Badge tone="pink">{eyebrow}</Badge><h1>{title}</h1><p>{description}</p></div>{children}</section>;
}

function NotFound() {
  return <RoutePage eyebrow="404" title="This route escaped into the vents." description="No page exists here. The raccoon denies involvement."><Button tone="acid" onClick={() => navigate("/")}>Go home</Button></RoutePage>;
}
